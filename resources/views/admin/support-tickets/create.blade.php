                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('admin.support-tickets.index') }}">Support
                                    Tickets</a></li>
        ...
        <section class="content">
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-12">
                        <div class="card">
                            <form action="{{ route('admin.support-tickets.store') }}" method="post" id="submitForm"
                                    ...
                                    <div class="row">
                                        <div class="col-lg-3 col-md-3 col-sm-12">
                                            <div class="form-group">
                                                ...
                                                <select name="user_id" class="select2b4 form-control" required>
                                                    <option value="">Select Client</option>
                                                    @foreach ($usersDataSet as $userData)
                                                        <option value="{{ $userData->id }}"
                                                            {{ $userData->id == old('user_id') ? 'selected' : '' }}>
                                                            {{ $userData->first_name }}
                                                            {{ $userData->last_name }}</option>
                                                    @endforeach
                                                </select>
                                                <small class="help-text text-muted">Create ticket on behalf of
                                                    client</small>
                                            </div>
                                        </div>
                                        <div class="col-lg-3 col-md-3 col-sm-12">
                                            <div class="form-group">
                                                ...
                                                <select name="ticket_status_id" class="form-control select2b4" required>
                                                    <option value="">Select Status</option>
                                                    @foreach ($ticketStatusData as $status)
                                                        <option value="{{ $status->id }}"
                                                            {{ $status->id == old('ticket_status_id') ? 'selected' : '' }}>
                                                            {{ $status->name }}</option>
                                                    @endforeach
                                                </select>
                                                <small class="help-text text-muted">Select ticket status</small>
                                            </div>
                                        </div>
                                        <div class="col-lg-3 col-md-3 col-sm-12">
                                            <div class="form-group">
                                                <label for="support_ticket_priority_id">Priority</label>
                                                <select name="support_ticket_priority_id" class="form-control select2b4"
                                                    id="support_ticket_priority_id" required>
                                                    <option value="">Select Priority</option>
                                                    @foreach ($ticketPriorityData as $priority)
                                                        <option value="{{ $priority->id }}"
                                                            {{ $priority->id == old('ticket_status_id') ? 'selected' : '' }}>
                                                            {{ $priority->name }}</option>
                                                    @endforeach
                                                </select>
                                                <small class="help-text text-muted">Select ticket priority</small>
                                            </div>
                                        </div>
                                        <div class="col-lg-3 col-md-3 col-sm-12">
                                            <div class="form-group">
                                                <label for="is_locked">Locked?</label>
                                                <select name="is_locked" class="form-control select2b4" id="is_locked"
                                                    required>
                                                    <option value="">Select Option</option>
                                                    <option value="0" {{ old('is_locked') == 0 ? 'selected' : '' }}>No
                                                    </option>
                                                    <option value="1" {{ old('is_locked') == 1 ? 'selected' : '' }}>
                                                        Yes</option>
                                                </select>
                                                <small class="help-text text-muted">E.g. Yes = internal ticket and will not
                                                    appear at client dashboard</small>
                                            </div>
                                        </div>
                                ...
                                <div class="card-footer">
                                    ...
                                    <a href="{{ route('admin.support-tickets.index') }}"
                                        class="btn btn-flat btn-default"><i class="fas fa-times"></i>
                                        Discard</a>
                                </div>
                                <!-- /.card-footer -->
                            </form>
